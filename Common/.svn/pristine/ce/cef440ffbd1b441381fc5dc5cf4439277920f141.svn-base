...
begin
  ...
  begin
    ...
    begin
      try
        if (Field is TWideMemoField) or
           (Field is TBlobField) then
        begin
          // caution : Lines is a property of self
          Lines.Text := TWideMemoField(Field).AsString;
        end
        else
          if (Mask = '') or (Field is TWideStringField) then
...
begin
  ...
  begin
    ...
    begin
      FieldNo := Field.Index;
      FieldOK := true;
      if (Field is TWideMemoField) or (Field is TBlobField) then
      begin
        FPrintCaption := '';
        IsMemo := true;
      end
...
begin
  ...
  begin
    Field := FDataSet.FindField(FDataField);
    if (Field is TBlobField) or (Field is TWideMemoField) then
    begin
      Caption := '';
    end;
  end else

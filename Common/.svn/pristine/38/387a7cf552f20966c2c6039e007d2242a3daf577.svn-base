...
type
  ...
  EMathParserError = class(Exception); { create a new exception class and... }
    ...
    TCustomParser = class(TComponent)private
  ...
  public
    ...
    property Variable[const VarName: string]
      : extended read GetVariable write SetVar;
  published
    ...
    property PascalNumberformat
      : boolean read FPascalNumberformat write FPascalNumberformat default
      true;
    property OnParserError
      : TParserExceptionEvent read FOnParserError write FOnParserError;
  end;

  TMathParser = class(TCustomParser)
  private
  ...
  public
    ...
    property ParseString: String read FExpression write SetExpression;
  published
    { predefined variables - could be left out }
    {
      property A;
...
procedure Register;
begin
  RegisterComponents(SOFTWARE_NAME, [TMathParser]);
end;
(*
  {$IFDEF VER80}
  {$R *.D16}
      ...
      begin
        case Token of
          ...
          FuncOneVar, FuncTwoVar:
            { job has been done in build already ! } ;
        end; { case }
...
function TCustomParser.SetVariable(VarName: string;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        dec(i);
        AString := Strings[i];
        if (Length(AString) <> 1) or (not(AString[1] in ['A' .. 'E', 'T', 'X',
            'Y'])) then
        begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (Length(AVarName) <> 1) and (not(AVarName[1] in ['A' .. 'E', 'T',
            'X', 'Y'])) then
  ...
  begin
    ...
    begin
      ...
      begin
        if IndexOfObject(TObject(Arg1)) >= 0 then
          Arg1 := nil;
        if IndexOfObject(TObject(Arg2)) >= 0 then
          Arg2 := nil;
        if IndexOfObject(TObject(Dest)) >= 0 then
          Dest := nil;
...
{$ENDIF}
    ...
    procedure TMathParser.Parse; begin try FParseValue := CalcValue;
    FParseError := false; except FParseError := true; end; end;

    class function TMathParser.RemoveBlanks(const s: string): string;
    { deletes all blanks in s }
    var i: integer; begin Result := s;

    i := pos(' ', Result); while i > 0 do begin Delete(Result, i, 1);
    i := pos(' ', Result); end; end;

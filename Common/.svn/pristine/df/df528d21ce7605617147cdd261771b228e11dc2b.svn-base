...
function VineToDelphiDateTime(const dt: string; out Value: TDateTime): Boolean;
function DelphiToVineDateTime(const dt: TDateTime): string;
...
function VineToDelphiDateTime(const dt: string; out Value: TDateTime): Boolean;
var
  yy, mt, dd, hh, mm, ss: string;
  fst: TFormatSettings;
  val: TDateTime;
begin
  Result := False;
  if (Length(dt) = 19) then
  begin
    if (Copy(dt, 5, 1) = '-')
      and (Copy(dt, 8, 1) = '-')
      and (Copy(dt, 11, 1) = ' ')
      and (Copy(dt, 14, 1) = ':')
      and (Copy(dt, 17, 1) = ':') then
      begin
        yy := Copy(dt, 1, 4);
        mt := Copy(dt, 6, 2);
        dd := Copy(dt, 9, 2);
        ...
        mm := Copy(dt, 15, 2);
        ss := Copy(dt, 18, 2);
        if IsNumeric(yy + mt + dd + hh + mm + ss) then
        begin
          try
            fst.DateSeparator := '-';
            fst.TimeSeparator := ':';
            fst.ShortDateFormat := 'yyyy-MM-dd';
            fst.LongTimeFormat := 'HH:mm:ss';
            val := StrToDateTime(dt, fst);
            if DelphiToVineDateTime(val) = dt then
            begin
              Value := val;
              Result := True;
            end;
...
begin
  ...
  case VType of
    ...
    varDate:     Result := TSuperObject.Create(DelphiToVineDateTime(vDate));
...
begin
  ...
  begin
    str := Copy(Value, i, 1);
    if (str <> '0') and (str <> '1') and (str <> '2') and (str <> '3')
      and (str <> '4') and (str <> '5') and (str <> '6') and (str <> '7')
      and (str <> '8') and (str <> '9') then
    begin
      Result := False;
      Break;
    end;

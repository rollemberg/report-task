...
type
  ...
  function Service(Sender: TComponent; const AService: string): IAppService;
  procedure ServiceLog(const AService: string; TickCount: Int64);
  function HttpObject(const service: string): IHttpObject;
  //登录到地藤系统，可重复调用
  function loginToVine(Sender: TComponent): Boolean;
  //写入日志
...
begin
  ...
  try
    ...
    begin
      ...
      OutputMessage(Self, Format('已取出 %d 条数据，开始取第 %d 次数据，请稍等！',
        [DataOut.RecordCount, FetchCount]), MSG_WARING);
      Screen.Cursor := crHourGlass;
      Application.ProcessMessages;
      Sleep(300);
      if FetchCount >= 50 then
      begin
        raise Exception.Create('对不起，系统最多允许一次导出25万笔数据，详情请与客服联系！');
        Break;
      end;
    end;
    if FetchCount > 1 then
      OutputMessage(Self, Format('数据全部取出，共执行 %d 次，总计 %d 笔！',
        [FetchCount, DataOut.RecordCount]), MSG_WARING);
  finally
    Screen.Cursor := def;
  end;
end;
...
var
  app3: TAppService3;
begin
  app3 := TAppService3.Create(Sender);
...
begin
  ...
  app := Service(Sender, 'TAppLogin.Check');
...
//            mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
          begin
            ...
            raise Exception.Create(Chinese.AsString('非常抱歉！暂时无法连接应用服务器，请检查您的网络连接后再尝试重新登录系统！'));
            Break;
          end;
        end;
      end;
